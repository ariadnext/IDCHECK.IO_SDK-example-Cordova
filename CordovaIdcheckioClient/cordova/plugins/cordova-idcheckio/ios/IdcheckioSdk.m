...
#import "CordovaIdcheckioClient-Swift.h"
...
- (void) activate:(CDVInvokedUrlCommand*)command{/Users/mlegendre/devel/ariadnext/Hybride/cordova-smartsdk/CordovaIdcheckioClient/platforms/ios/CordovaIdcheckioClient/Plugins/cordova-idcheckio/IdcheckioKeys.h
    NSString* licenceFileName = command.arguments[0];
    BOOL extractData = command.arguments[1];
    callback = command.callbackId;/Users/mlegendre/devel/ariadnext/Hybride/cordova-smartsdk/cordova-idcheckio/ios/IdcheckioSdk+Utils.swift
        ...
        if([key isEqualToString:DocumentType]){
            ...
            for(NSString* extraKey in [extraParams allKeys]){
                if([extraKey isEqualToString:Language]){
                    Idcheckio.shared.extraParameters.language = [json objectForKey:extraKey];
                } else if([extraKey isEqualToString:ManualButtonTimer]){
                    Idcheckio.shared.extraParameters.manualButtonTimer = [json doubleForKey:extraKey];
                } else if([extraKey isEqualToString:MaxPictureFilesize]){
                    [Idcheckio.shared.extraParameters setMaxPictureFilesize:[json integerForKey:extraKey]];
                } else if([extraKey isEqualToString:FeedbackLevel]){
                    [Idcheckio.shared.extraParameters setFeedbackLevel:[json objectForKey:extraKey]];
                } else if([extraKey isEqualToString:Token]){
                    [Idcheckio.shared.extraParameters setToken:[json objectForKey:extraKey]];
                } else if([extraKey isEqualToString:ConfirmAbort]){
                    Idcheckio.shared.extraParameters.confirmAbort = [json boolForKey:extraKey];
                } else if([extraKey isEqualToString:AdjustCrop]){
                    Idcheckio.shared.extraParameters.adjustCrop = [json boolForKey:extraKey];
                }
            }
        }
    }
